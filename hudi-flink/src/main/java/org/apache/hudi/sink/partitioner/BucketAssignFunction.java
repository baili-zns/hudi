...
public class BucketAssignFunction<K, I, O extends HoodieRecord<?>>
    ...
    private void processRecord(HoodieRecord<?> record, Collector<O> out) throws Exception {
        if (record instanceof HoodieRecordWithSchema) {
            HoodieRecordWithSchema hoodieRecordWithSchema = (HoodieRecordWithSchema) record;
            RowType rowType1 = hoodieRecordWithSchema.getRowType();
            List primaryKeyColumnNames1 = hoodieRecordWithSchema.getPrimaryKeyColumnNames();
